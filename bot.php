...
mysql_select_db($config['mysql']['dbname'],$db);

$channel_query = mysql_query('SELECT id,channel FROM channels ORDER BY channel ASC') or die('Could not get channel data.');
$channel_data = array();
while($channel = mysql_fetch_assoc($channel_query)) {
	$channel_data[$channel['id']] = $channel['channel'];
}

$ignore_query = mysql_query('SELECT nick FROM ignore_nick') or die('Could not get ignore data.');
$ignore_nicks = array($config['nickname']);
while($ignore = mysql_fetch_assoc($ignore_query)) {
		...
		if(time()-$last_write > $config['write_freq']) {
			...
			if(count($update_sql)) {
				...
				foreach($update_sql as $s) {
					...
					if(!mysql_query($s)) {
						$update_errors = true;
						$log_manager->write_log('error',mysql_errno().': '.mysql_error());
					}
				}
				if($update_errors) {
...
class StatTotals {
			...
			foreach($channels as $chan => $nicks) {
				foreach($nicks as $nick => $quantity) {
					$sql[] = 'INSERT INTO `'.$type.'`'
						.' SET channel_id="'.mysql_real_escape_string($chan).'", nick="'.mysql_real_escape_string($nick).'", total="'.mysql_real_escape_string($quantity).'"'
						.' ON DUPLICATE KEY UPDATE total=total+"'.mysql_real_escape_string($quantity).'"';
				}
			}
		}
		return $sql;
	...
	function reset_totals() {
		$this->_data = array(); // Just empty the _data array - leaving 0 values will generate useless queries otherwise
	}
}
			...
			foreach($channels as $chan => $hours) {
				foreach($hours as $hour => $quantity) {
					$sql[] = 'INSERT INTO `active_times`'
						.' SET channel_id="'.mysql_real_escape_string($chan).'", nick="'.mysql_real_escape_string($nick).'", hour="'.mysql_real_escape_string($hour).'", total="'.mysql_real_escape_string($quantity).'"'
						.' ON DUPLICATE KEY UPDATE total=total+"'.mysql_real_escape_string($quantity).'"';
				}
			}
		}
		...
		foreach($this->_data as $nick => $channels) {
			foreach($channels as $chan => $quote) {
				$sql[] = 'INSERT INTO `latest_quote`'
					.' SET channel_id="'.mysql_real_escape_string($chan).'", nick="'.mysql_real_escape_string($nick).'", quote="'.mysql_real_escape_string($quote).'"'
					.' ON DUPLICATE KEY UPDATE quote="'.mysql_real_escape_string($quote).'"';
			}
		}
		return $sql;
	}
		...
		foreach($this->_data as $nick => $channels) {
			foreach($channels as $chan => $totals) {
				$sql[] = 'INSERT INTO `textstats`'
					.' SET channel_id="'.mysql_real_escape_string($chan).'", nick="'.mysql_real_escape_string($nick).'",'
					.' messages="'.mysql_real_escape_string($totals['messages']).'", words="'.mysql_real_escape_string($totals['words']).'", chars="'.mysql_real_escape_string($totals['chars']).'",'
					.' avg_words='.mysql_real_escape_string($totals['words']).'/'.mysql_real_escape_string($totals['messages']).', avg_chars='.mysql_real_escape_string($totals['chars']).'/'.mysql_real_escape_string($totals['messages'])
					.' ON DUPLICATE KEY UPDATE messages=messages+"'.mysql_real_escape_string($totals['messages']).'", words=words+"'.mysql_real_escape_string($totals['words']).'", chars=chars+"'.mysql_real_escape_string($totals['chars']).'", avg_words=words/messages, avg_chars=chars/messages';
			}
		}
		return $sql;
	}
...
class MonologueMonitor {
	private $_lastspoke_nick = array();
	private $_lastspoke_count = array();
