        ...
        */
        ...
        switch ($message->command) {
            ...
            case self::CMD_JOIN:
                ...
                $this->emit("join, join:$channel, join:$nick, join:$channel:$nick", [
                    'nick' => $nick,
                    'channel' => $channel
                ]);
                break;
            case self::CMD_PART:
                ...
                $this->emit("part, part:$channel, part:$nick, part:$channel:$nick", [
                    'nick' => $nick,
                    'channel' => $channel
                ]);
                break;
            case self::CMD_MODE:
                $nick = $message->nick ? $message->nick : $this->nick;
                $channel = $message->getArg(0);
                $modeString = $message->getArg(1);
                $changes = [];

                // What mode changes? Who is affected?
                $is_plus = false;
                $nickOffset = 1;
                for ($i = 0, $j = strlen($modeString); $i < $j; $i++) {
                    switch ($modeString[$i]) {
                        ...
                        default:
                            $nickOffset++;
                            $changes[] = [($is_plus ? '+' : '-'), $modeString[$i], $message->getArg($nickOffset)];
                    }
                }

                $this->emit("mode, mode:$channel, mode:$channel:$nick", [
                    'nick' => $nick,
                    'channel' => $channel,
                    'changes' => $changes,
                ]);
            ...
            case self::CMD_KICK:
                //Emit kick events
                $channel = $message->getArg(0);
                $victim = $message->getArg(1);
                $kicker = $message->nick;

                $this->emit("kick, kick:$channel, kick:$victim, kick:$channel:$victim", [
                    'victim' => $victim,
                    'kicker' => $kicker,
                    'channel' => $channel
                ]);
            ...
            case self::CMD_NOTICE:
                ...
                $this->emit("notice, notice:$to, notice:$to:$from", [
                    'from' => $from,
                    'to' => $to,
                    'text' => $text
                ]);
            ...
            case self::CMD_PRIVMSG:
                ...
                if ($this->isChannel($to)) {
                    ...
                    $this->emit("chat, chat:$to, chat:$to:$from", [
                        'from' => $from,
                        'channel' => $to,
                        'text' => $text
                    ]);
